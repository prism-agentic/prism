...
## UI 优化 — Hero 终端固定窗口 + 内容滚动
- [x] 终端窗口大小固定不变，内容超出时自动向上滚动（新行从底部出现，旧行向上移出）
- [x] 保存 checkpoint
...
## Phase 15 — 清理代码中的 manus 引用

- [x] 扫描项目中所有包含 manus 的文件和引用
- [x] 分析并清理不必要的 manus 引用（10 处已清理）
- [x] 验证编译通过（135 个测试全部通过）
- [ ] 重新推送到 GitHub（待执行）
